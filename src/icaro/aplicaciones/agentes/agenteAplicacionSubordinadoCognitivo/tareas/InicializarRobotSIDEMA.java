...
public class InicializarRobotSIDEMA extends TareaSincrona {
	...
	public void ejecutar(Object... params) {
		...
		try {
			...
			boolean found = false;
			ItfUsoRecursoVisualizacionSIDEMA visualizador = (ItfUsoRecursoVisualizacionSIDEMA) NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ
					.obtenerInterfaz(NombresPredefinidos.ITF_USO + "RecursoVisualizacionSIDEMA1");
			for (int i = 0; i < nList.getLength() && !found; i++) {
				Node nNode = nList.item(i);
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					Element eElement = (Element) nNode;
					// String id =
					// ((Node)eElement.getElementsByTagName("id").item(0)).getTextContent();
					String id = ((Node) eElement.getElementsByTagName("id").item(0)).getTextContent();
					if (id.equals(miIdentAgte)) {
						found = true;
						String type = eElement.getAttribute("type");
						int energy = Integer
								.parseInt(((Node) eElement.getElementsByTagName("energy").item(0)).getTextContent());
						...
						if (type.equalsIgnoreCase("Explorador")) {
							int timeMov = Integer.parseInt(
									((Node) eElement.getElementsByTagName("movementTime").item(0)).getTextContent());
							int enerMov = Integer.parseInt(
									((Node) eElement.getElementsByTagName("movementEnergy").item(0)).getTextContent());
							int timeExp = Integer.parseInt(
									((Node) eElement.getElementsByTagName("explorationTime").item(0)).getTextContent());
							int enerExp = Integer
							...
							int enerMov = Integer.parseInt(
									((Node) eElement.getElementsByTagName("movementEnergy").item(0)).getTextContent());
							int timeExp = Integer
									.parseInt(((Node) eElement.getElementsByTagName("desactivationTime").item(0))
											.getTextContent());
							int enerExp = Integer
							...
							this.trazas.trazar(miIdentAgte, "No se ha reconocido el tipo del agente",
			...
			if (!found)
				this.trazas.trazar(miIdentAgte, "No se ha encontrado el fichero de inicializacion de Estatus",
						InfoTraza.NivelTraza.error);
		} catch (Exception e) {
			e.printStackTrace();
		}
